...
namespace MissionPlanner.GCSViews
{
    public partial class FlightPlanner : MyUserControl, IDeactivate, IActivate
    {
        ...
        //Landing setup
        bool LandingPointMode = false;
        public bool landingStripMode = false;
        public int landingStripPointCount = 0;
        PointLatLngAlt beginningOfRunway = new PointLatLng(0, 0);
        PointLatLngAlt endOfRunway = new PointLatLng(0, 0);
        ...
        public void setfromMap(double lat, double lng, int alt, double p1 = -1)
        {
            ...
            if (alt != -1 && alt != -2 && Commands.Columns[Alt.Index].HeaderText.Equals("Alt"))
            {
                ...
                if ((Commands.Rows[selectedrow].Cells[Command.Index].Value.ToString() != "VTOL_LAND") && (Commands.Rows[selectedrow].Cells[Command.Index].Value.ToString() != "LAND"))
                {
...
Column 1: Field type (RALLY is the only one at the moment -- may have RALLY_LAND in the future)
 ...
 Column 7: Flags (just 0 for now, also future use).
        ...
        private void wpClear_Click(object sender, EventArgs e)
        {
            if(cmb_missiontype.Text == "MISSION" && polygongridmode == false){
                clearMissionToolStripMenuItem_Click(this, null);
            }
            if (cmb_missiontype.Text == "MISSION" && polygongridmode == true)
            {
                clearPolygonToolStripMenuItem_Click(this, null);
            }
            else if (cmb_missiontype.Text == "FENCE" && polygongridmode == false)
            {
                clearMissionToolStripMenuItem_Click(this, null);
            }
            if (cmb_missiontype.Text == "FENCE" && polygongridmode == true)
            {
                clearPolygonToolStripMenuItem_Click(this, null);
            }
            else if (cmb_missiontype.Text == "RALLY" && polygongridmode == false)
            {
                clearRallyPointsToolStripMenuItem_Click(this, null);
            }
            if (cmb_missiontype.Text == "RALLY" && polygongridmode == true)
            {
                clearPolygonToolStripMenuItem_Click(this, null);
            }

            Cmb_missiontype_SelectedIndexChanged(null, null);
        }

        private void setupTakeoff_Click(object sender, EventArgs e)
        {
        ...
        private void TakeoffSetup()
        {
            ...
            if (MainV2.CurrentUAV.isFW == 1 && MainV2.CurrentUAV.isVTOL == 1)
            {
                ...
                int VtolAlt = (int)(MainV2.CurrentUAV.vtolTKoffAlt * CurrentState.multiplieralt);
            ...
            else if (MainV2.CurrentUAV.isFW == 1 )
            {
                ...
                int tkoffAlt = (int)(MainV2.CurrentUAV.missionAltFW * CurrentState.multiplieralt);
                InputBox.Show("Desired Takeoff Alt", "Enter Desired Takeoff Alt in " + CurrentState.AltUnit, ref tkoffAlt);
            ...
            {
                int tkoffAlt = (int)(MainV2.CurrentUAV.vtolTKoffAlt * CurrentState.multiplieralt);
                InputBox.Show("Desired Takeoff Alt", "Enter Desired Takeoff Alt in " + CurrentState.AltUnit, ref tkoffAlt);
        ...
        public void SetupLandingStrip()
        {
            ...
            if (landingStripPointCount == 1) //first point of a new runway
            {
                ...
                beginningOfRunway = new PointLatLngAlt(MouseDownEnd.Lat, MouseDownEnd.Lng,0);
            ...
            {
                    LandingDirection = MainMap.MapProvider.Projection.GetBearing(endOfRunway, beginningOfRunway);
                    SetupLandingWaypoints();
            }
        }
    }
